...
class NodeResolver {
  ...
  getPackageEntries(pkg, env) {
    ...
    let mainFields = [pkg.source, browser]; // If scope hoisting is enabled, we can get smaller builds using esmodule input, so choose `module` over `main`.
    ...
    return mainFields.filter(entry => typeof entry === 'string').map(main => {
